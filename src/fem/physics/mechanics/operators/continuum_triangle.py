...
from ....elements.tri6.definition import (
...
class Tri3LinearOperator:
    """Tri3 plane stress/strain linear operator."""
    canonical_type = "Tri3"
    aliases = ("CPS3", "CPE3")
    ...
    def linear_contribution(
        ...
        *,
        ...
        """Return the v2 stateless Tri3 contribution."""

        del time, temperature
        reference = _plane_coordinates(
            reference_coordinates,
            "reference_coordinates",
            node_count=3,
        )
        local_displacement = _plane_coordinates(
            displacement,
            "displacement",
            node_count=3,
        )
        ...
        if len(dof_tuple) != 6:
            raise ValueError(
                f"Tri3 element {getattr(element, 'id', '?')} requires 6 DOFs; "
                f"got {len(dof_tuple)}"
            )
        source_properties = (
        ...
        B, area = self._B_matrix_from_reference(element, reference)
        D = linear_elastic.plane_matrix(
            normalized.E,
            normalized.nu,
            normalized.plane_type,
        )
        tangent = normalized.thickness * area * (B.T @ D @ B)
        strain = B @ local_displacement.reshape(-1)
        output_batch = LocalOutputBatch.element(
            int(element.id),
            (
    ...
    def _B_matrix_from_reference(
        self,
        elem: Any,
        reference_coordinates: np.ndarray,
    ) -> tuple[np.ndarray, float]:
        """Return the constant Tri3 B matrix from owned reference geometry."""
    ...
    def _B_matrix(
        ...
        return self._B_matrix_from_reference(elem, reference)
...
def _plane_coordinates(
    ...
    *,
    ...
    """Validate finite two-component coordinates for a plane element."""

    try:
        coordinates = np.asarray(value, dtype=float)
    except (TypeError, ValueError) as exc:
        raise ValueError(f"{name} must be a numeric array") from exc
    if coordinates.shape != (node_count, 2) or not np.all(np.isfinite(coordinates)):
        raise ValueError(
            f"{name} must be a finite array with shape ({node_count}, 2); "
...
class Tri6LinearOperator:
    """Tri6 plane stress/strain linear operator."""
    canonical_type = "Tri6"
    aliases = ("CPS6", "CPE6")
