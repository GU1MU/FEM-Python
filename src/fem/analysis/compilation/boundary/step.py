...
from fem.model.targets import resolve_displacement_node_ids
...
def boundary_for_step(model: Any, step: str | int | AnalysisStep | None = None) -> BoundaryCondition:
    ...
    constraint_selector = (
        selected_step.name
        if isinstance(selected_step, AnalysisStepSnapshot)
        else selected_step
    )
    for constraint in effective_displacement_constraints(model, constraint_selector):
