...
class SketchGeometry:
    ...
    def __post_init__(self) -> None:
        ...
        if not contours or not all(isinstance(item, SKETCH_CONTOUR_TYPES) for item in contours):
...
class MovedGeometry:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.base, (*BASE_GEOMETRY_TYPES, MovedGeometry, RotatedGeometry, ExtrudedGeometry, BooleanGeometry)):
...
class ExtrudedGeometry:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.base, (*BASE_GEOMETRY_TYPES, MovedGeometry, RotatedGeometry, BooleanGeometry)):
...
class BooleanGeometry:
    ...
    def __post_init__(self) -> None:
        ...
        supported = (*BASE_GEOMETRY_TYPES, MovedGeometry, RotatedGeometry, ExtrudedGeometry, BooleanGeometry)
        if not isinstance(self.object_geometry, supported) or not isinstance(self.tool_geometry, supported):
            raise TypeError("布尔操作需要两个已有几何")
        if geometry_dimension(self.object_geometry) != geometry_dimension(self.tool_geometry):
