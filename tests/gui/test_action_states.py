...
def test_actions_follow_document_and_result_context(gui_inp_path):
    ...
    assert (
        window.actions["geometry_create"].toolTip()
        == window.actions["geometry_create"].text()
    )
    assert not window.actions["geometry_sketch"].isEnabled()
    assert (
        window.actions["geometry_sketch"].toolTip()
        == window.actions["geometry_sketch"].text()
    )
...
def test_every_disabled_action_keeps_plain_label_without_reason():
    _application()
    window = FEMMainWindow()
    ...
    decorated = [
        ...
        and (
            action.toolTip().strip() != action.text().strip()
            or action.statusTip().strip() != action.text().strip()
            or "（" in action.toolTip()
        )
    ]
...
def test_load_action_uses_the_same_dimension_filtered_regions_as_dialog():
    ...
    assert (
        ...
        == window.actions["load_create"].text()
    )
    window.close()
...
def test_truss_member_policy_disables_only_local_mesh_control():
    ...
    assert (
        window.actions["mesh_local_control"].toolTip()
        == window.actions["mesh_local_control"].text()
    )
    ...
    assert not window.actions["mesh_generate"].isEnabled()
    assert (
        window.actions["mesh_generate"].toolTip()
        == window.actions["mesh_generate"].text()
    )
    assert window.actions["mesh_controls"].isEnabled()
    window.close()
