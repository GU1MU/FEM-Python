...
@pytest.mark.integration
def test_repository_perforated_plate_is_queried_after_solve(tmp_path):
    source = (
        Path(__file__).resolve().parents[1]
        / "data"
        / "perforated_plate.inp"
    )
    workspace = tmp_path / "workspace"
    artifacts = ArtifactStore(workspace)
    session_id = artifacts.create_session("ses_perforated_plate")
    artifact = artifacts.copy_input(session_id, source)
    revisions = RevisionStore(workspace)
    revisions.create_session(session_id)
    record = revisions.initialize(
        ...
        idempotency_key="initialize_perforated_plate",
    )
    ConfirmationStore(workspace, revisions).confirm(
        session_id,
        revision=record.revision,
    ...
    response = IsolatedFEMWorker(workspace).run(
        ...
        revision=record.revision,
        revision_hash=record.revision_hash,
        idempotency_key="solve_perforated_plate",
        timeout_seconds=60,
    )
    summary = IsolatedFEMResultQuerier(workspace).query(
        response,
        (
            ResultQuery(
                ResultQueryKind.MAX_DISPLACEMENT_MAGNITUDE,
    ...
    assert scalar.node_id == 4
    assert scalar.value == pytest.approx(0.503840529776, rel=1e-9)
    unicode_summary = IsolatedFEMResultQuerier(workspace).query(
        response,
        (
            ResultQuery(
                ResultQueryKind.MAX_DISPLACEMENT_MAGNITUDE,
    ...
    assert unicode_summary.scalars == ()
    assert len(unicode_summary.diagnostics) == 1
    assert "自由端" in unicode_summary.diagnostics[0].message
    assert len(
        list(
            (
                artifacts.session_path(session_id)
                / "runs"
            ).iterdir()
        )
    ) == 1
